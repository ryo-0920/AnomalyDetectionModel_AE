...
try:
    ...
    if exc.name not in (None, "seaborn"):
        raise
    sns = None
# ============================
# 共通ユーティリティ
...
def _draw_boxplot(
    ...
    if sns is not None:
        ...
        return
    print(f"[INFO] seaborn is not installed; using matplotlib fallback for {y_col} boxplot.")
    valid = data[[x_col, y_col]].copy()
    ...
    labels: List[str] = []
    for label, group in valid.groupby(x_col, sort=False, dropna=False):
        values = group[y_col].to_numpy(dtype=float)
        if values.size == 0:
            continue
        groups.append(values)
        labels.append("nan" if pd.isna(label) else str(label))
    if not groups:
        ax.text(0.5, 0.5, "No valid data", ha="center", va="center", transform=ax.transAxes)
        return
...
def _draw_heatmap(
    ...
    if sns is not None:
        ...
        return
    print(f"[INFO] seaborn is not installed; using matplotlib fallback for {value_col} heatmap.")
    values = pivot.to_numpy(dtype=float)
    cmap = plt.get_cmap("viridis").copy()
    cmap.set_bad("#f2f2f2")
    image = ax.imshow(np.ma.masked_invalid(values), cmap=cmap, vmin=0.0, vmax=1.0, aspect="auto")
    ...
    for row_idx in range(values.shape[0]):
        for col_idx in range(values.shape[1]):
            ...
            if np.isfinite(value):
                ...
                ax.text(col_idx, row_idx, f"{value:.2f}", ha="center", va="center", color=color)
# ============================
# ON / OFF 固定設定（ラベル・台帳）
# ============================
...
def plot_margin_boxplots(
    ...
    _draw_boxplot(ax1, data=df, x_col=group_col, y_col="margin_sec")
    ...
    _draw_boxplot(ax2, data=df, x_col=group_col, y_col="margin_ratio_A1A2")
...
def plot_macro_heatmap(
    ...
    _draw_heatmap(ax, pivot=pivot, value_col=value_col)
...
def main() -> None:
    ...
    ap.add_argument("--config", help="ON/OFF 共通の設定ファイル (JSON) のパス")
    ap.add_argument("--config_on", help="ON (事故) 用 設定ファイル (JSON) のパス")
    ap.add_argument("--config_off", help="OFF (正常) 用 設定ファイル (JSON) のパス")
    ...
    if args.config and (args.config_on or args.config_off):
        ap.error("--config cannot be used together with --config_on or --config_off")
    if not args.config and not (args.config_on and args.config_off):
        if args.config_on and not args.config_off:
            ap.error("--config_off is required when --config_on is used without --config")
        if args.config_off and not args.config_on:
            ap.error("--config_on is required when --config_off is used without --config")
        ap.error("either --config or both --config_on and --config_off are required")

    if args.config:
        cfg_path = Path(args.config)
        eval_on = load_evaluation_config(cfg_path)
        eval_off = eval_on
        cfg_on_path = cfg_path
        cfg_off_path = cfg_path
    else:
        cfg_on_path = Path(args.config_on)
        cfg_off_path = Path(args.config_off)
        eval_on = load_evaluation_config(cfg_on_path)
        eval_off = load_evaluation_config(cfg_off_path)
